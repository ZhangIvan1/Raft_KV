...
type Raft struct {
	...
	log *RaftLog // enable log compressed
...
func (rf *Raft) becomeLeaderLocked() {
	...
	for peer := 0; peer < len(rf.peers); peer++ {
		...
		rf.nextIndex[peer] = rf.log.size() // enable log compressed
...
func (rf *Raft) becomeCandidateLocked() {
	...
	LOG(rf.me, rf.currentTerm, DVote, "%d become a Candidate for Term %d", rf.me, rf.currentTerm+1)
	rf.currentTerm++
	rf.role = Candidate
	rf.voteFor = rf.me
...
func (rf *Raft) becomeFollowerLocked(term int) {
	...
	LOG(rf.me, rf.currentTerm, DLog, "%d become a Follower, from Term %d to Term %d", rf.me, rf.currentTerm, term)
	rf.role = Follower
	// do persist when currentTerm change
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.log = NewLog(InvalidIndex, InvalidIndex, nil, nil)
